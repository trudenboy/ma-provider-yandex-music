...
- Aligned the `ya-passport-auth[ma]` manifest pin with the locked and integration dependency version so the provider can be synced safely.
...
- Stabilized Yandex login timeout coverage and retained library items with a valid provider id when parsing fails.
